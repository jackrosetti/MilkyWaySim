#NOTE: I have used so many sources for this project. It's amazing, really. I will list all of them below:
'''
http://nova.stanford.edu/projects/mod-x/ad-kepler.html, https://en.wikipedia.org/wiki/Semi-major_and_semi-minor_axes,
https://en.wikipedia.org/wiki/Sphere_of_influence_(astrodynamics), https://en.wikipedia.org/wiki/Eccentric_anomaly,
https://www.youtube.com/watch?v=ND5PSKpZDKE, http://jwilson.coe.uga.edu/EMAT6680Fa05/Bacon/hohmanntransfers.html
...
class Planet():
    ...
    def calculate_init_position(self):
    ...
    def calculate_init_velocity(self, delta):
        ...
        velocity = np.sqrt(G * M * (2./self.distance_to_sun() - 1./self.semiMajor))
        return (velocity * np.cos(self.eccentricAnomaly) - G * M * self.position[0]/self.distance_to_sun()**3 * delta/2,
                velocity * np.sin(self.eccentricAnomaly) - G * M * self.position[1]/self.distance_to_sun()**3 * delta/2)
    ...
    def update_velocity(self, delta):
        ...
        return tuple(i - (delta * G * M * ri) / self.distance_to_sun()**3 for i, ri in zip(self.velocity, self.position))
...
class SolarSystemSimulation():
    ...
    def __init__(self):
        ...
        for key, planet in SolarSystemSimulation.Planets.copy().items():
            if planet.simulate or key in [self.originPlanet, self.destinationPlanet]:
                ...
                planet.position = planet.calculate_init_position()
                planet.velocity = planet.calculate_init_velocity(self.delta)
    ...
    def timeStep(self):
        ...
        for planet in SolarSystemSimulation.Planets.values():
            planet.position = planet.update_position(self.delta)
            planet.velocity = planet.update_velocity(self.delta)
...
class Spacecraft(Planet):
    ...
    def __init__(self, origin, destination, delta):
        ...
        self.velocity = self.calculate_init_velocity(delta)
    ...
    def calculate_init_velocity(self, delta):
        ...
        return (velocity*np.cos(self.eccentricAnomaly) - G*M*self.origin.position[0]/self.origin.distance_to_sun()**3 * delta/2,
                velocity*np.sin(self.eccentricAnomaly) - G*M*self.origin.position[1]/self.origin.distance_to_sun()**3 * delta/2)

    def perform_orbital_insertion(self, Sim):
        if not Sim.Planets['HTO'].orbitInserted:
            ...
            Sim.Planets['HTO'].velocity = (Sim.Planets['HTO'].velocity[0] + requiredAcceleration*(Sim.Planets['HTO'].velocity[0]/Sim.Planets['HTO'].get_velocity()),
                                           Sim.Planets['HTO'].velocity[1] + requiredAcceleration*(Sim.Planets['HTO'].velocity[1]/Sim.Planets['HTO'].get_velocity()))
...
class App:
                ...
                if gui.plotHohmann.get():
                    if gui.orbitInsertion.get():
                        ...
                        if distanceToDestination < sphereOfInfluence:
                            Sim.Planets['HTO'].perform_orbital_insertion(Sim)
    ...
    def hohmannTransfer(self, Sim):
        ...
        for i, planet in enumerate([Sim.Planets[Sim.originPlanet], Sim.Planets[Sim.destinationPlanet]]):
            ...
            planet.position = planet.calculate_init_position()
            planet.velocity = planet.calculate_init_velocity(Sim.delta)
